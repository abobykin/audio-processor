(function() {
  ...
  var frequencies = window.frequencies;
  ...
  class AudioProcessor {
    constructor() {
      ...
      this.strings = {
        e2: {
          offset: Math.round(this.audioContext.sampleRate / frequencies.e2),
          difference: 0
        },
        a2: {
          offset: Math.round(this.audioContext.sampleRate / frequencies.a2),
          difference: 0
        },
        d3: {
          offset: Math.round(this.audioContext.sampleRate / frequencies.d3),
          difference: 0
        },
        g3: {
          offset: Math.round(this.audioContext.sampleRate / frequencies.g3),
          difference: 0
        },
        b3: {
          offset: Math.round(this.audioContext.sampleRate / frequencies.b3),
          difference: 0
        },
        e4: {
          offset: Math.round(this.audioContext.sampleRate / frequencies.e4),
          difference: 0
        }
      };
      ...
      if(time < this.assessStringsUntilTime) {
        ...
        for(let o = 0;o < this.stringsKeys.length;o++) {
          ...
          for(let i = 0;i < searchSize;i++) {
            difference += Math.abs(this.frequencyBuffer[i] - this.frequencyBuffer[i + offset]);
          }
